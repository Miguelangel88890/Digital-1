...
*/
void init_adc(void){
	adc1_adc_stream_rstrb_write(1);
	adc1_adc_stream_spi_tx_data_write(0x0218);
	while( !(adc1_adc_stream_done_tx_read() & 1) );
	adc1_adc_stream_rstrb_write(0);
	adc1_adc_stream_rstrb_write(1);
	adc1_adc_stream_spi_tx_data_write(0x0218);
	while( !(adc1_adc_stream_done_tx_read() & 1) );
 	adc1_adc_stream_rstrb_write(0);
	adc1_adc_stream_sample_counter_write(2336);
}
...
void read_samples(void){
	...
	adc1_adc_stream_st_capture_write(1);
	adc1_adc_stream_st_capture_write(0);

/*
	while( !(adc1_adc_stream_done_capture_read() & 1 )  )


	adc1_adc_stream_re_b_write(1);
    address = 0;
	for(j=0; j < 2048; j++){
		for(i = 0; i < 8; i ++){
		  adc1_adc_stream_adr_b_write(address);
		  if(i==2){
		    data=adc1_adc_stream_dat_b_read();
			write_memory_32( ( 0x40500000  + j*2 ) , (  data ) );
		  }
		  address ++;
		}
	}
	adc1_adc_stream_re_b_write(0);
*/
	adc1_adc_stream_st_capture_write(1);
...
void send_data_to_UDP(void){
    // Configure DMA
	dma_upload_dma_reader_enable_write(0);
	dma_upload_dma_reader_base_write(0x40500000);
	dma_upload_dma_reader_length_write(2048);
	dma_upload_dma_reader_enable_write(1);
//	dma_upload_dma_reader_enable_write(0);

    // Configure UDP destination IP and port   
	dma_upload_udp_streamer_ip_address_write(0x0a2a0064);
	dma_upload_udp_streamer_enable_write(0);
	dma_upload_dma_reader_loop_write(0);
	dma_upload_udp_streamer_enable_write(1);
    my_busy_wait(100);
	dma_upload_dma_reader_loop_write(1);



	printf("DMA base: %lx\n",      dma_upload_dma_reader_base_read() );
	printf("DMA length: %lx\n",    dma_upload_dma_reader_length_read() );
	printf("DMA enable: %lx\n",    dma_upload_dma_reader_enable_read() );
	printf("DMA done: %lx\n",      dma_upload_dma_reader_done_read() );
	printf("UDP dest_ip: %lx\n",   dma_upload_udp_streamer_ip_address_read() );
	printf("UDP dest_port: %lx\n", dma_upload_udp_streamer_udp_port_read() );
	printf("UDP enable: %lx\n",    dma_upload_udp_streamer_enable_read() );
