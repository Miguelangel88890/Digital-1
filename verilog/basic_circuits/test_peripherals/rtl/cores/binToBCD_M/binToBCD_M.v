module binToBCD_M(clk, rst, init, done, A, pp);
    ...
    acc_binToBCD_M acc_M0 (.clk(clk), .i(i) , .addi(w_sh) , .rst(w_reset) , .c(w_C));

    main_process_binToBCD_M main_process_M0 (.clk(clk) , .rst(w_reset) , .in_A(A) , .A1(temp_SUM1) , .A2(temp_SUM2) , .A3(temp_SUM3) , .s_A(w_A) , .sh(w_sh) , .ld1(w_ld1) , .ld2(w_ld2) , .ld3(w_ld3));
