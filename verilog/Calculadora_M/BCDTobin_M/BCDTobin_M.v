module BCDTobin_M(clk, rst, init, done, A, B , pp);
    ...
    main_process_M main_process_M0 (.clk(clk) , .rst(rst) , .in_A(A) , .in_B(B) , .A1(temp_Menos1) , .A2(temp_Menos2) , .A3(temp_Menos3) , .s_A(w_A) , .sh(w_sh) , .ld1(w_ld1) , .ld2(w_ld2) , .ld3(w_ld3));
